...
class STCNModel:
    def __init__(self, para, logger=None, save_path=None, local_rank=0, world_size=1):
        ...
        self.save_model_interval = 5000 # 再次训练找最好的模型
        # self.save_model_interval = 100 # 测试用
        if para['debug']:
    ...
    def do_pass(self, data, it=0):
        ...
        out = {}
        # 预训练时：stage_0
        # data['rgb']    (B, 帧数, 3, H, W)  B = batch_size = 8, 帧数 = 3,  H = W = 384
        # data['gt]      (B, 帧数, 1, H, W)  B = batch_size = 8, 帧数 = 3,  H = W = 384 
        Fs = data['rgb']
        Ms = data['gt']
        ...
        with torch.cuda.amp.autocast(enabled=self.para['amp']):
            ...
            k16, kf16, kf8, kf4 = self.STCN('encode_key', Fs)

            if self.single_object:  # 单目标
                # 预训练时：stage_0
                # input：（B, 3, H, W） (B, 1024, H/16, W/16) (B, 1, H, W) 
                # output: (B, 512, T, H/16, W/16)  frame 0 的 value  T = 1
                ref_v = self.STCN('encode_value', Fs[:,0], kf16[:,0], Ms[:,0])

                # Segment frame 1 with frame 0
                # 预训练时：stage_0
                # input：（B, 64, H/16, W/16） (B, 512, H/16, W/16) (B, 512, H/8, W/8) (B, 256, H/4, W/4) 
                #        （B, 64, 1, H/16, W/16） (B, 512, H/16, W/16)
                # output:logits:  (B, 1, H, W) (B, 1, H, W) mask = softmax(logits)  
                prev_logits, prev_mask = self.STCN('segment', 
                        k16[:,:,1], kf16[:,1], kf8[:,1], kf4[:,1],   # k16[:,:,1] frame 1 的 key
                        k16[:,:,0:1], ref_v)                              # k16[:,:,0:1] frame 0 的 key(memory key)  
                ...
                this_logits, this_mask = self.STCN('segment', 
                        k16[:,:,2], kf16[:,2], kf8[:,2], kf4[:,2],  # k16[:,:,2] frame 2 的 key
                        k16[:,:,0:2], values)                            # k16[:,:,0:2] frame 0 and 1 的 key(memory key)
            ...
            if self._is_train:
                ...
                if it % self.save_model_interval == 0 and it != 0: # 正常训练
                # if it >= 200000 and it % 10000 == 0: # retrain
                    if self.logger is not None:
                        self.save(it)
            ...
            #####################################################################
            # for name, param in self.STCN.named_parameters():
    ...
    def load_network(self, path):
        ...
        for k in list(src_dict.keys()):
            if k == 'value_encoder.conv1.weight':
                # 预训练时输入value_encoder.conv1 的是 frame+mask（dim=4）
                # 而主训练存在多目标，输入value_encoder.conv1 的是 frame+mask+other_mask (dim=5)
                # 故主训练加载预训练模型时，需要将预训练模型的权重文件修改，加一个权重全为0的通道
